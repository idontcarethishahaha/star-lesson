...
spring:
  application:
    name: aigc-service
  datasource:
    url: jdbc:mysql://127.0.0.1:3306/tj_aigc?useUnicode=true&amp;characterEncoding=UTF-8&amp;serverTimezone=Asia/Shanghai
    username: root
    password: root
  data:
    redis:
...
**推荐模型搭配**：
- 主模型：`glm-4.5-flash`（推理强、速度快、价格低）
- 向量模型：`embedding-3`（语义检索效果好）
